...
public class ReaperWandInteraction {
	public static void Handle(final PlayerInteractEvent e) {
		...
		if (p.getInventory().getItemInMainHand() != null && p.getInventory().getItemInMainHand().hasItemMeta()
			...
			if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
				...
				p.getWorld().spawnParticle(Particle.SQUID_INK, p.getLocation(), 250, 0.5, 0.0, 0.5, 0.05);
				...
				if (wand.getItemMeta().hasLore()) {
					...
					loreItems.set(0, Extensions.GetNextSpell(WandSpellLists.ReaperWandSpells, wand, p));
				} else {
					loreItems.add(WandSpellLists.ReaperWandSpells.get(0));
				}
